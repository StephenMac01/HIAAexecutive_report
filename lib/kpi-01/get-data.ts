...
export async function getKpi01Data(): Promise<Kpi01Data> {
  ...
  const events: KpiEvent[] = rawRows.map((r) => ({
    ...
    treatment: String(r.Treatment).trim() === "Included" ? "Included" : "Excluded",
  ...
  const countedEvents = events.filter((e) => e.substantiated && e.treatment === "Included")
  ...
  const categoryBreakdown: Kpi01CategoryDatum[] = CATEGORY_ORDER.map((category) => {
    ...
    return {
      ...
      short: category
        ...
        .map((w) => w[0])
        .join(""),
      counted: inCategory.filter((e) => e.substantiated && e.treatment === "Included").length,
      excluded: inCategory.filter((e) => !(e.substantiated && e.treatment === "Included")).length,
      total: inCategory.length,
    }
  }).filter((c) => c.total > 0)

  const sourceBreakdown: Kpi01SourceDatum[] = Object.entries(countBy(events.map((e) => e.source))).map(
    ([source, total]) => {
      const counted = events.filter(
        (e) => e.source === source && e.substantiated && e.treatment === "Included",
      ).length
  ...
  const cumulativeTimeline: Kpi01TimelineDatum[] = (() => {
    ...
    return MONTHS.slice(0, 8).map((label, idx) => {
      ...
      return { month: label, counted: lastCounted, points: lastPoints, threshold: kpiMeta.failThreshold }
    })
  })()

  return { events, summary, categoryBreakdown, sourceBreakdown, cumulativeTimeline }
}
