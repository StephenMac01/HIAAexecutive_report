...
export async function getExecutiveSummary(): Promise<ExecutiveSummary> {
  ...
  // Run every adapter in parallel and isolate failures with allSettled so one
  // bad workbook can never reject the whole Executive Summary. A rejected (or
  // missing) adapter becomes an `unavailable` contribution downstream.
  const settled = await Promise.allSettled(ids.map((id) => ADAPTERS[id]?.() ?? Promise.reject(new Error("no adapter"))))
  const raws = settled.map((result, i): { id: string; raw: RawContribution | null } => {
    const id = ids[i]
    if (result.status === "fulfilled") return { id, raw: result.value }
    console.log(
      `[kpi] exec-summary adapter failed for ${id}:`,
