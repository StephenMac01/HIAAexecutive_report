...
function defaultCacheSeconds(): number {
  const raw = Number(process.env.KPI_CACHE_TTL_SECONDS ?? process.env.KPI_CACHE_SECONDS)
...
function kpiDrivePath(kpiId: string): string {
  ...
  const template = process.env.SHAREPOINT_FILE_TEMPLATE?.trim() || "{id}/{id}.xlsx"
  return template
    .replace(/\{id\}/g, kpiId)
    .replace(/\\/g, "/")
    .replace(/\/{2,}/g, "/")
...
export type KpiSourceDiagnostics = SharePointDiagnostics & {
  kpiId: string
  parse: { ok: boolean; detail: string; sheetNames?: string[]; dataRows?: number }
}
 ...
 */
export async function diagnoseKpiSource(kpiId: string): Promise<KpiSourceDiagnostics> {
  ...
  let parse: KpiSourceDiagnostics["parse"] = { ok: false, detail: "skipped (download did not succeed)" }
  if (sp.overallOk) {
    try {
      const bytes = await downloadSiteFile(drivePath, { cacheSeconds: 0 })
      const wb = parseWorkbookBuffer(bytes)
      const firstSheet = wb.Sheets[wb.SheetNames[0]]
      const dataRows = firstSheet ? Math.max(0, Object.keys(firstSheet).filter((k) => /^A\d+$/.test(k)).length - 1) : 0
      parse = {
        ok: true,
      ...
      parse = { ok: false, detail: error instanceof Error ? error.message : String(error) }
    }
  }
