 ...
 *
 * This route has NO managed-platform dependency — `revalidateTag` is a standard
 * Next.js API that works identically on a self-hosted Node.js server, a Windows
...
export async function POST(request: Request) {
  ...
  if (derivedId) {
    ...
    revalidateTag(kpiCacheTag(derivedId), {
      expire: 0,
    });
  ...
  for (const kpi of KPIS) {
    revalidateTag(kpiCacheTag(kpi.id), {
      expire: 0,
    });
  }
