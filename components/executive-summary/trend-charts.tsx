        ...
        <CardContent>
          <ChartContainer config={pointsConfig} className="h-[240px] w-full">
            <BarChart data={portfolioMonthly} margin={{ left: 4, right: 8, top: 8 }}>
              ...
              <Bar dataKey="damage" fill="var(--color-damage)" radius={[3, 3, 0, 0]} />
              <Bar dataKey="advantage" fill="var(--color-advantage)" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ChartContainer>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <ChartContainer config={pointsConfig} className="h-[240px] w-full">
            <AreaChart data={portfolioMonthly} margin={{ left: 4, right: 8, top: 8 }}>
              ...
              <YAxis tickLine={false} axisLine={false} fontSize={11} width={32} />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Area type="monotone" dataKey="net" stroke="var(--color-net)" strokeWidth={2} fill="url(#es-net)" />
            </AreaChart>
          </ChartContainer>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <ChartContainer config={pointsConfig} className="h-[240px] w-full">
            <LineChart data={rolling} margin={{ left: 4, right: 8, top: 8 }}>
              ...
              />
              <Line type="monotone" dataKey="rolling" stroke="var(--color-rolling)" strokeWidth={2} dot={false} />
            </LineChart>
          </ChartContainer>
        </CardContent>
      </Card>
        ...
        <CardContent className="flex items-center">
          <ChartContainer config={pointsConfig} className="mx-auto h-[240px] w-full">
            <PieChart>
              ...
              <Pie data={statusMix} dataKey="count" nameKey="label" innerRadius={55} outerRadius={90} paddingAngle={2}>
                {statusMix.map((s) => (
